/* Landing page renderer. All content comes from data/*.json so the page stays
   in sync with the portal crawl (build/fetch_igvf.py) and the manuscript
   legend extraction (build/extract_legends.py).

   data/filesets.json  page structure: tiers, donor matrix, per-set file lists
   data/files.json     flat index of all 1,813 files, filtered by the builder
   data/figures.json   Figure 1's legend
*/
...
function renderMatrix(d) {
  const order = ["C29", "C39", "C37", "C38", "A8"];
  const byName = Object.fromEntries(
    Object.values(d.donor_meta).map((v) => [v.name, v]));
  const donors = order.filter((n) => byName[n]);

  const head = `<thead><tr><th style="text-align:left">Donor</th>${
    d.sets.map((s) => `<th>${esc(s.label)}</th>`).join("")}</tr></thead>`;

  const body = donors.map((name) => {
    const m = byName[name];
    const flag = m.efficiency === "not in manuscript";
    const pill = m.efficiency === "efficient" ? '<span class="pill eff">high efficiency</span>'
      : m.efficiency === "inefficient" ? '<span class="pill ineff">low efficiency</span>'
      : `<span class="pill">${esc(m.note)}</span>`;
    const cells = d.sets.map((s) => s.donors.includes(m.accession)
      ? '<td class="yes">●</td>' : '<td class="no">·</td>').join("");
    return `<tr class="${flag ? "flag" : ""}">
        <th><a href="${esc(m.portal)}" target="_blank" rel="noopener">${esc(name)}</a> ${pill}
          <small>${esc(m.accession)} · ${esc(m.sex || "")}</small></th>${cells}</tr>`;
  }).join("");
...
function renderRecipes(d) {
  ...
  document.getElementById("recipes").innerHTML = `
    <div class="recipes">
      <div class="recipe">
        <h4>One file</h4>
        <p>The URL is stable and needs no login. <code>-C -</code> resumes a
           partial file, which matters for the BAMs.</p>
        ${codeBlock(`curl -fL -C - -O "${h5ad.download}"`)}
      </div>

      <div class="recipe">
        <h4>Everything of one type, live from the portal</h4>
        <p>The manifest is regenerated by the portal on every request, so this
           picks up files released after this page was built.</p>
        ${codeBlock(manifestPipe(mo.intermediate.types[0].manifest))}
      </div>

      <div class="recipe">
        <h4>From S3, no HTTP redirect</h4>
        <p><code>igvf-public</code> is a public bucket; the AWS CLI is faster
           than curl for many large files and parallelises per file.</p>
        ${codeBlock(`aws s3 cp --no-sign-request "${h5ad.s3_uri}" .

# or many at once, from a manifest column
cut -f11 manifest.tsv | tail -n +2 \\
  | xargs -n1 -P4 -I{} aws s3 cp --no-sign-request {} ./`)}
      </div>

      <div class="recipe">
        <h4>Verify what you downloaded</h4>
        <p>Every manifest carries the portal's md5 for each file.</p>
        ${codeBlock(`# column 9 is the md5, column 10 the URL whose basename is the local name
awk -F'\\t' 'NR>1{n=$10; sub(/.*\\//,"",n); print $9"  "n}' manifest.tsv > md5s.txt
md5sum -c --ignore-missing md5s.txt   # macOS: brew install coreutils, then gmd5sum`)}
      </div>

      <div class="recipe">
        <h4>Walk the tiers yourself</h4>
        <p>Each principal set lists its inputs; each of those lists theirs. The
           JSON API mirrors the page exactly.</p>
        ${codeBlock(`curl -sH 'Accept: application/json' \\
  "https://api.data.igvf.org/analysis-sets/${mo.accession}/?format=json" \\
  | jq -r '.input_file_sets[]."@id"'`)}
      </div>
